...
import type { EventType, MessageType, MiraiApiHttpSetting, MiraiOptions } from '.';
 ...
 * 数据类型
 */
export type Data<T extends 'message' | EventType.EventType | MessageType.ChatMessageType> =
...
export class Mirai {
  ...
  async release() {
    ...
    if (data.code === 0) this.logger.success(`释放 ${this.qq} Session: ${this.api.sessionKey}`);
   ...
   * @param callback
   */
  _adaptMessageForAll<T extends 'message' | EventType.EventType | MessageType.ChatMessageType>(
    method: 'on' | 'off' | 'once',
  ...
  async reply(
    ...
    if (isChatMessage(srcMsg))
      if (quote && srcMsg.messageChain[0].type === 'Source') messageId = srcMsg.messageChain[0].id;

    // 自动转化为数组
    if (typeof msgChain !== 'string' && !Array.isArray(msgChain)) msgChain = [msgChain];
    ...
    if (typeof msgChain === 'string') {
      ...
      if (sections.length > 1) {
        ...
        for await (const section of sections) {
          const res = await this._sendMessageByType(type, section, target, messageId);
          responses.push(res);
        }
        return responses;
      }
  ...
  _sendMessageByType(
    ...
    if (this.defaultAppendMessage) {
      if (typeof msgChain === 'string') msgChain += this.defaultAppendMessage;
      else msgChain.push({ type: 'Plain', text: this.defaultAppendMessage });
    }
    if (type === 'friend') return this.api.sendFriendMessage(msgChain, target, messageId);
    else if (type === 'group') return this.api.sendGroupMessage(msgChain, target, messageId);
  }
